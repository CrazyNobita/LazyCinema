...
async def who_is(client, message):
    ...
    message_out_str += (
        f"<b>➲ User Link:</b> "
        f"<a href='tg://user?id={from_user.id}'>"
        f"<b>Click Here</b></a>\n"
    )
...
async def dreamxbotz_list_movies(client, message):
    try:
        ...
        logger.error(
            f"Error in dreamxbotz_list_movies: {e}"
        )
...
async def dreamxbotz_list_series(client, message):
    try:
        ...
        for i, (title, seasons) in enumerate(
            series_data.items(),
            1
        ):
            season_list = ", ".join(
                f"{s}" for s in seasons
            )

            msg += (
                f"<b>{i}. {title} - "
                f"Season {season_list}</b>\n"
            )
        ...
        logger.error(
            f"Error in dreamxbotz_list_series: {e}"
        )
